...
class AgentNetwork(nn.Module):
    ...
    def mutate(self, mutation_rate: float = 0.1) -> "AgentNetwork":
        instance = AgentNetwork(self.input_size, self.hidden_size, self.output_size)

        with torch.no_grad():
            for (name, param), (instance_name, instance_param) in zip(
                self.named_parameters(), instance.named_parameters()
            ):
